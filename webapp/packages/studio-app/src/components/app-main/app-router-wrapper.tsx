...
const ProjectListView = lazy(
  () => import("../project-resource/project-list/project-list-view")
);
      ...
      <Suspense fallback={<LoadingIndicator />}>
        <Routes>
          <Route path={RoutePath.ROOT} element={<AppRouterLandingLayout />}>
            ...
            <Route
              path={`${RoutePath.USER_ACTIVATE}/:key`}
              element={<UserActivate />}
            />
            <Route
              path={`${RoutePath.USER_PASSWORD_RESET_CONFIRM}/:uid/:token`}
              element={<UserPasswordChange resetConfirm={true} />}
            />
            <Route
              path={`${RoutePath.USER_PASSWORD_CHANGE}`}
              element={<UserPasswordChange resetConfirm={false} />}
            />
            ...
            />
            <Route
              path={RoutePath.PROJECT_PLAYGROUND}
