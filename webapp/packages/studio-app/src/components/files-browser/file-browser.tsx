...
const FileBrowser: React.FC<FilesTreeProps> = ({
  reduxProject,
  ...
  useEffect(() => {
    ...
    if (projectFiles && projectFiles.length) {
      ...
      const rootNode: FileReduxNode = {
        ...getFileTreeFromReduxFileList(reduxProject.title, projectFiles),
      };
      if (debugComponent) {
  ...
  useEffect(() => {
    ...
    if (reduxProject.entryFileLocalId) {
      ...
      if (debugLocalOnlyPendingSupport) {
        console.log(
          `attribute 'entryFileLocalId' not set in projet '${reduxProject.title}'`
        );
      }
    }
  ...
  const handleFileBrowserControlBarEvent = (
    event: FileBrowserControlBarEvent
  ) => {
    let newFilePath = "src";
    ...
    switch (event.name) {
      ...
      case FileBrowserControlBarEventType.UPLOAD_FILES:
        ...
        if (reduxProject.selectedFileLocalId) {
          const selectedFile = getFileFromLocalId(
            filesState,
            reduxProject.selectedFileLocalId
          );
          if (selectedFile) {
  ...
  const handleFileComponentTreeClick: ItemClickFunc = (keyName, itemInfo) => {
    if (debugComponent) {
      console.log(
        `FileBrowser:handleFileComponentTreeClick()  keyName:${keyName} itemInfo:`,
        itemInfo
      );
    }
  ...
  const handleFileComponentTreeEvent: ItemEventFunc = (type, data) => {
    if (debugComponent) {
      console.log(
        `FileBrowser:handleFileComponentTreeEvent()  type:${type} data:`,
        data
      );
    }
    ...
    switch (type) {
      case "change":
        if (data.itemInfo.value.info.type === "folder") {
          console.log(
            `Folder '${data.itemInfo.value.info.name}' name change not supported`
          );
          break;
        }
      ...
      default:
        console.error(
          `FileBrowser:handleFileComponentTreeEvent()  event type '${type}' not supported`
        );
    }
  };

  const [dragStartFileInfo, setDragStartFileInfo] = useState<FileInfo | null>(
    null
  );
  ...
  const handleDragStart = (itemInfo: ItemInfoType) => {
    if (debugComponent || false) {
      if (itemInfo.value.info.type === "file") {
        ...
        console.log(
          `handleDragStart():`,
          itemInfo.value.info.rootNamePath.join("/")
        );
      }
    }
  ...
  const handleDrop = (itemInfo: ItemInfoType) => {
    ...
    if (itemInfo.value.info.type === "folder") {
      ...
      dstFolder = getFilePathParts(itemInfo.value.info.reduxFile.path)[
        "dirname"
      ];
    }
    ...
    if (dragStartFileInfo) {
      ...
      if (debugComponent) {
        console.log(
          `srcFilePath:'${srcFilePath}' dstFilePath:'${dstFilePath}'`
        );
      }

      if (srcFilePath === dstFilePath) {
        console.log(
          `The srcFilePath:${srcFilePath} is same as dstFilePath:${dstFilePath}`
        );
        return;
      }

      if (propOnEvent) {
        propOnEvent("path-change", {
          localId: dragStartFileInfo.reduxFile.localId,
          newPath: dstFilePath,
        });
      }
    }
